...
def discover_oidc_endpoints(discovery_url):
    ...
    try:
        ...
        if missing:
            logger.error(f"[OIDC] Discovery document missing required endpoints: {missing}")
            return None
        
        logger.info(f"[OIDC] Successfully discovered endpoints from {endpoints.get('issuer', 'provider')}")
...
def configure_oidc_settings():
    ...
    if discovery_url:
        ...
        if endpoints:
            setattr(django_settings, 'OIDC_OP_AUTHORIZATION_ENDPOINT', endpoints['authorization_endpoint'])
            setattr(django_settings, 'OIDC_OP_TOKEN_ENDPOINT', endpoints['token_endpoint'])
            setattr(django_settings, 'OIDC_OP_USER_ENDPOINT', endpoints['userinfo_endpoint'])
            ...
            logger.error("[OIDC] Discovery failed, falling back to manual configuration if available")
    ...
    setattr(django_settings, 'OIDC_AUTHENTICATION_BACKEND', 
            'pretalx_oidc.auth.PretalxOIDCBackend')
    
    # Optional: Provider name for logging/display
    setattr(django_settings, 'OIDC_PROVIDER_NAME', 
    ...
    logger.info(f"  - Callback URL name: {django_settings.OIDC_AUTHENTICATION_CALLBACK_URL}")
    logger.info(f"  - Auth Backend Class: {django_settings.OIDC_AUTHENTICATION_BACKEND}")
    ...
    if hasattr(django_settings, 'OIDC_OP_AUTHORIZATION_ENDPOINT'):
        logger.info(f"  - Authorization: {django_settings.OIDC_OP_AUTHORIZATION_ENDPOINT}")
