...
from pretalx.cfp.signals import html_head as cfp_html_head, html_above_profile_page
...
def add_oidc_login_button(sender, request, next_url=None, **kwargs):
    ...
    ''')
        ...
        result = '\n'.join(html_parts)
        logger.info(f"[OIDC] Returning HTML (length: {len(result)}, has <style>: {'<style>' in result})")
...
def add_cfp_css(sender, request, **kwargs):
    ...
    logger.info(f"[OIDC] cfp_html_head signal received - sender: {sender}, path: {request.path if request else 'no request'}")
...
def add_orga_css(sender, request, **kwargs):
    ...
    logger.info(f"[OIDC] orga_html_head signal received - sender: {sender}, path: {request.path if request else 'no request'}")
    # This signal is called for both event-specific and global pages
...
def add_profile_css(sender, request, **kwargs):
    ...
    """
    logger.info(f"[OIDC] html_above_profile_page signal received - sender: {sender}, path: {request.path if request else 'no request'}")
    if should_hide_password_form():
